...
export default function FillBlanksPage() {
  ...
  const [gameResult, setGameResult] = useState<{
    correct: number; total: number; timeRemaining: number; maxCombo: number; hintsUsed: number;
  } | null>(null);

  const deck = getDeckById(settings.selectedDeckId);
  const totalTime = getTimerForDifficulty(settings.difficulty);

  const handleComplete = useCallback((correct: number, total: number, timeRemaining: number, maxCombo: number, hintsUsed: number) => {
    const score = calculateScore(correct, total, timeRemaining, totalTime, maxCombo, hintsUsed);
    setGameResult({ correct, total, timeRemaining, maxCombo, hintsUsed });
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-3xl font-bold text-gray-900">Fill in the Blanks</h1>
        </div>
    ...
    <div className="max-w-2xl mx-auto px-4 py-8">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-400">Fill in the Blanks - {settings.difficulty}</p>
        </div>
      </div>
      <FillBlankGame cards={deck.cards} difficulty={settings.difficulty} onComplete={handleComplete} />
    </div>
  );
}
